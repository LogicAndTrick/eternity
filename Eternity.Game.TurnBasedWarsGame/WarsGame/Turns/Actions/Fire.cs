...
namespace Eternity.Game.TurnBasedWarsGame.WarsGame.Turns.Actions
{
    ...
    public class Fire : IUnitAction, IUnitActionGenerator
    {
        ...
        {
            return set.CurrentMoveSet.Any()
                && set.CurrentMoveSet.All(x => x.MoveType != MoveType.Attack)
                && set.Unit.GetAttackableTiles(set.CurrentMoveSet.Last().MoveTile, set.CurrentMoveSet.GetMovementCost()).Any();
        }
